...
public class CameraView extends FrameLayout implements LifecycleObserver {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        try {
            if (mInEditor) {
                final int width = Math.max(MeasureSpec.getSize(widthMeasureSpec), 1);
                final int height = Math.max(MeasureSpec.getSize(heightMeasureSpec), 1);
            ...
            if (mLastPreviewStreamSize == null) {
                int w = Math.max(widthMeasureSpec, 1);
                int h = Math.max(heightMeasureSpec, 1);
                Log.d("onMeasure:", "surface is not ready. Calling default behavior. width " + w + " height " + h);
                super.onMeasure(w, h);
                return;
            }
            ...
            final int widthValue = Math.max(MeasureSpec.getSize(widthMeasureSpec), 1);
            final int heightValue = Math.max(MeasureSpec.getSize(heightMeasureSpec), 1);
            final float previewWidth = Math.max(mLastPreviewStreamSize.getWidth(), 1);
            final float previewHeight = Math.max(mLastPreviewStreamSize.getHeight(), 1);
            ...
            Log.d("onMeasure:", "requested dimensions are ("
                    + widthValue + "[" + ms(widthMode) + "]x"
                    + heightValue + "[" + ms(heightMode) + "])");
            Log.d("onMeasure:", "previewSize is ("
            ...
            if (widthMode == EXACTLY && heightMode == EXACTLY) {
                Log.d("onMeasure:", "both are MATCH_PARENT or fixed value. We adapt. This means CROP_CENTER. (" + widthValue + "x" + heightValue + ")");
            ...
            if (widthMode == UNSPECIFIED && heightMode == UNSPECIFIED) {
                Log.d("onMeasure:", "both are completely free. We respect that and extend to the whole preview size. (" + previewWidth + "x" + previewHeight + ")");
                super.onMeasure(
            ...
            if (widthMode == UNSPECIFIED || heightMode == UNSPECIFIED) {
                ...
                Log.d("onMeasure:", "one dimension was free, we adapted it to fit the ratio. (" + width + "x" + height + ")");
                super.onMeasure(MeasureSpec.makeMeasureSpec(width, EXACTLY),
                        MeasureSpec.makeMeasureSpec(height, EXACTLY));
                return;
            }
            ...
            if (widthMode == EXACTLY || heightMode == EXACTLY) {
                ...
                Log.d("onMeasure:", "one dimension was EXACTLY, another AT_MOST We have TRIED to fit the aspect ratio, but it's not guaranteed.  (" + width + "x" + height + ")");
                super.onMeasure(MeasureSpec.makeMeasureSpec(width, EXACTLY),
                        MeasureSpec.makeMeasureSpec(height, EXACTLY));
                return;
            }
            ...
            Log.d("onMeasure:", "both dimension were AT_MOST. We fit the preview aspect ratio. (" + width + "x" + height + ")");
            super.onMeasure(MeasureSpec.makeMeasureSpec(width, EXACTLY),
                    MeasureSpec.makeMeasureSpec(height, EXACTLY));
        } catch (Exception e) {
            e.printStackTrace();
            Log.d("onMeasure:", "error ----------------------------------- ");
            int width = Math.max(widthMeasureSpec, 1);
            int height = Math.max(heightMeasureSpec, 1);
